...
network_edge_sex_differences <- function(metadata, tissue_edge_filtered, file_name= "Tissue_GTEX_sex"){
  ...
  test_BH <- LVTestWrapperJLF(b.matrix=tissue_edge_filtered, sample.info.df= sampleinfo, phenotype.col= "sex", blocking= RIN, file.lead= file_name, plot.dir= paste0(dir_path,"/output/lioness/differential_edge_analysis"), results.dir= paste0(dir_path,"/output/lioness/differential_edge_analysis"), use.bonferroni= FALSE, sig.threshold = 0.05)
  return( test_BH)
}
```
...
```{r}
Liver_lioness_res <- readRDS(paste0(dir_path,"output/lioness/adjusted_lioness_output/Liver_lioness_res_all.rds"))
Liver_gtex_metadata <- readRDS(paste0(dir_path,"data/gtex_qsmooth/Liver_gtex_metadata.rds"))
```
```{r}
...
```{r}
Liver_tissue_panda_m <- readRDS("/data/project/lasseigne_lab/JLF_scratch/Sex_Bias_Adverse_Events/output/alpaca/sex_specific_networks/Liver_M_panda.rds")
Liver_tissue_panda_f <- readRDS("/data/project/lasseigne_lab/JLF_scratch/Sex_Bias_Adverse_Events/output/alpaca/sex_specific_networks/Liver_F_panda.rds")
```

```{r}
top_regNet_m  <- as.data.frame(Liver_tissue_panda_m@regNet)
top_regNet_m$TF_name<- rownames(top_regNet_m)
top_regNet_long_m<- pivot_longer(top_regNet_m, cols=!TF_name, names_to = "Gene", values_to = "weight_male")
top_regNet_long_m$Gene<-  str_replace_all(top_regNet_long_m$Gene, "-", ".")
ids_m<- paste(top_regNet_long_m$Gene, top_regNet_long_m$TF_name, sep= "_")
```

```{r}
top_regNet_f  <- as.data.frame(Liver_tissue_panda_f@regNet)
top_regNet_f$TF_name<- rownames(top_regNet_f)
top_regNet_long_f<- pivot_longer(top_regNet_f, cols=!TF_name, names_to = "Gene", values_to = "weight_female")
top_regNet_long_f$Gene<-  str_replace_all(top_regNet_long_f$Gene, "-", ".")
ids_f<- paste(top_regNet_long_f$Gene, top_regNet_long_f$TF_name, sep= "_")
```


```{r}
identical(ids_m, ids_f)
```

```{r}
diff <- abs(top_regNet_long_f$weight_female - top_regNet_long_m$weight_male)
filter_1 <- diff > 0.25

positive_edge <- (top_regNet_long_f$weight_female > 0 | top_regNet_long_m$weight_male > 0)

filter_f<- (filter_1 & positive_edge)

```


```{r}
Liver_lioness_res_filter <- Liver_lioness_res[filter_f, ]
```

```{r}
rm(Liver_tissue_panda_f, Liver_tissue_panda_m, Liver_lioness_res)
gc()
```

```{r}
limma_edge_liver <- network_edge_sex_differences(Liver_gtex_metadata, Liver_lioness_res_filter, file_name= "test14_liver_GTEX_sex")
```
