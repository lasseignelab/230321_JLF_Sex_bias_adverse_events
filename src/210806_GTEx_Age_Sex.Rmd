...
gtex_coldata$SEX_AGE <- paste(gtex_coldata$gtex.sex,
                              gtex_coldata$gtex.age, sep = "_")
```

```{r}
gtex_coldata$SEX_AGE_DEATH <- paste(gtex_coldata$SEX_AGE,
                                    gtex_coldata$gtex.dthhrdy, sep = "_")
```
...
for (i in seq_along(tissue)){
  ...
  plot <- ggplot(data = data,  aes(x = gtex.age,
      ...
      ggtitle(title)
  print(plot)
  nfile <- paste("~/output/GTEx_plots/210806_age_sex_GTEx_",
                 tissue[i], ".pdf", sep = "")
  ggsave(nfile)
}
```
...
```{r}
ggplot(data = age_sex_table_v2,  aes(x = gtex.age,
                                     y = Freq,
                                     group = interaction(gtex.sex,
                                     gtex.smts))) +
...
ggplot(autolysis_score, aes(fill = gtex.smatsscr, y = Freq, x = gtex.smtsd)) +
   geom_bar(position = "fill", stat = "identity") +
    ...
    ggtitle("Autolysis Score for GTEx tissue")
ggsave(
  "~/output/GTEx_plots/210806_autolysis_score_tissue_structure_GTEx_precent.pdf"
  )
```
